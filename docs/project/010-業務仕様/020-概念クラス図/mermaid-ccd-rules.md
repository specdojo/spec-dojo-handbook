...
- クラス数が 30 を超え可読性低下。
- ドメインサブ領域（例: 販売 / 会計 / 仕入）が明確化。
- 関係線が交差し視認性が低い。

手法：

- 共通基底（例: 「商品」「顧客」）を小さなコア図にまとめ、各領域図で参照関係のみ定義。
- 各領域図に凡例とルール参照リンクを必ず付与。

## 12. 状態とライフサイクル

概念の状態が複雑（10以上、遷移ルール多）な場合：

- 属性として `状態` を置くより、別途「状態遷移図」を作成。
- クラス図側は主要状態のみ列挙し冗長化を避ける。

簡易で十分な場合：

- `状態` 属性一つで管理（例: 登録 / 一部入金 / 完了）。
