...
用語集は、プロジェクト内で同じ言葉を同じ意味で使うための「辞書」であり、業務データ辞書（BDD）などの `glossaryTermId` が参照する **用語ID（`tm-...`）の正**になります。
...
- 形式は YAML とし、**トップレベルは `terms:` の配列**で統一します。
