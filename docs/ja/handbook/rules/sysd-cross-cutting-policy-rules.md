...
- 例外は SYSD-CCP 本文に散らさず、ADRで理由と影響範囲を管理する
...
| 例外        | SYSD-CCPの通常ルールから外れる運用。理由と期限をADRで明示する |
...
| 3    | 各ルール詳細（ID単位）                                          | ○    |
| 4    | 例外（ADRリンク）                                               | ○    |
| 5    | 関連ドキュメント導線（SDI/SYSD Critical Flows/NFR/OPD/OPR/ADR） | ○    |
...
- 「ルールは最小限」「検証可能性を優先」「例外はADRで管理」の原則を明記する。
...
生成する本文の見出しは **## 4. 例外（ADRリンク）**

- 例外の詳細本文は書かず、例外条件・影響範囲・期限・ADRリンクのみを記載する。
- 例外が恒久化しそうな場合は、SYSD-CCP本体ルールへの取り込み可否を定期レビューする。
- 例外未記載の実装差分は不適合として扱う。

### 6.5. 関連ドキュメント導線

生成する本文の見出しは **## 5. 関連ドキュメント導線（SYSD/SYSD-CF/NFR/OPD/OPR/ADR）**

- ルールごとに、SYSD（一次情報）、SYSD Critical Flows（重要フロー）、NFR、OPD/OPR、ADR への導線を明記する。
...
| Rule ID     | 例外条件                 | 影響範囲            | 期限       | ADR                                 |
| ----------- | ------------------------ | ------------------- | ---------- | ----------------------------------- |
| scp-RET-001 | 外部課金APIで再送が不可  | 決済機能のみ        | 2026-12-31 | `adr-00xx-payment-retry-exception`  |
| scp-MOD-001 | 移行期間中の一時依存許容 | legacy adapter のみ | 2026-06-30 | `adr-00yy-legacy-dependency-waiver` |

### 8.5. 関連ドキュメント導線（SYSD/SYSD-CF/NFR/OPD/OPR/ADR）
