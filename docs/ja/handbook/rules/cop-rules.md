...
- `mip-index`：移行計画のSSOT（カットオーバー概要・判定点の骨格を含む）
- `dmd-*`：データ移行設計（抽出/変換/投入/検証の詳細）
- `mtp-index` / `mtp-*`：移行リハーサル計画と結果（GO/NO-GO根拠）
- `cop-index`/ `cop-*`：本番切替手順のSSOT（分単位実行の入口）
- `otp-index` / `otp-*`：運用切替・ハイパーケア（監視/窓口/初動）
