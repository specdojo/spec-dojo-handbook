...
- ファイル名は `utc-010-単体テスト仕様-<term>.md` 等（例: `utc-010-単体テスト仕様-在庫.md`）。
...
注意：本章(6)配下の「6.x」番号はルール文書内の参照用です。生成する `utc-<term>` 本文の見出し番号ではありません。
...
生成する `utc-<term>` 本文の見出しは **### 6.6.1. 観点:&lt;観点&gt;**
