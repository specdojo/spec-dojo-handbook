...
- `<level>`はテストレベル略号（`ut（単体）/it（内部結合）/et（外部結合）/st（総合）/at（受入）`）を表す。
- `<term>` は対象単位を表す識別子（例：`inventory`, `product-register`）とする。

### 3.1. 記述内容とドキュメントの対応

| 分類      | 記述内容                                                                       | ドキュメント名               | ドキュメントID    |
| --------- | ------------------------------------------------------------------------------ | ---------------------------- | ----------------- |
| 全体      | テスト全体の目的・基本方針                                                     | テスト戦略・方針             | `tsp-overview`    |
| 全体      | テストレベル毎の品質ゲート（入口条件・出口条件）                               | テスト戦略・方針             | `tsp-overview`    |
| 全体      | テストの運用前提（環境・データ・自動化・不具合運用等）                         | テスト戦略・方針             | `tsp-overview`    |
| 全体      | テスト観点（What）と上位仕様へのトレース、適用レベルへの割当                   | テスト観点・条件             | `tpc-<term>`      |
| 全体      | テスト条件（When/State）と必要に応じて上位仕様へのトレース、適用レベルへの割当 | テスト観点・条件             | `tpc-<term>`      |
| 全体      | テスト条件に対する期待の要点（意味レベル、詳細な期待結果は下位へ）             | テスト観点・条件             | `tpc-<term>`      |
| `<level>` | `<level>`テストの対象範囲・境界、共通合格基準、対象別仕様へのナビゲーション    | `<level>`テスト仕様 全体構成 | `<level>s-index`  |
| `<level>` | `<level>`テストの対象別の範囲・境界、対象固有の追加合格基準（任意）            | `<level>`テスト仕様 対象別   | `<level>s-<term>` |
| `<level>` | `<level>`テストの対象別の代表ケース（任意）、証跡へのリンク                    | `<level>`テスト仕様 対象別   | `<level>s-<term>` |
| `<level>` | `<level>`テストの全ケースとテスト結果証跡                                      | コードとCI成果物（原則※）    |                   |

※　手動実施/監査/外部調整が必要な場合は、`<level>s-<term>` に最小限の手順・観測点チェックリストを追加で記載してもよい。

### 3.2. テスト戦略・方針 (TSP: Test Strategy / Policy; `tsp-overview`)

- テスト全体の目的／基本方針／役割分担（レベル別の責務境界）を定義する
- 品質ゲート（入口条件・出口条件）を定義する
- 環境・データ・自動化・不具合運用など、テスト運用の前提を定義する<br>
  ※ 個別の観点・条件・ケース・具体値は記載しない

### 3.3. テスト観点・条件 (TPC: Test Perspectives & Conditions; `tpc-<term>`)
