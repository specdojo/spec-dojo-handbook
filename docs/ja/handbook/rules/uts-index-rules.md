...
- 単体テストの責務（対象・境界・共通合格基準）
- TPC の観点・条件を `uts-<term>` に分配する方針
...
- 可能であれば、テスト戦略/方針の上位文書（例: `tsp-overview`）も `based_on` に含めます。
- `part_of` / `based_on` / `supersedes` は ID 配列で記載します（未指定は `[]` 可）。
...
対象単位（`uts-<term>`）の一覧・TPC 割当・個別仕様リンクは、**7.5 の「対象単位一覧（統合表）」** に集約します（SSOT。重複記載しません）。
...
- 個別仕様一覧は **7.5 の統合表（`uts-<term>` 列）** で管理します（重複禁止・命名ゆれ禁止）。
...
対象単位の一覧・TPC 割当・個別仕様リンクは 9.6 に集約する。
