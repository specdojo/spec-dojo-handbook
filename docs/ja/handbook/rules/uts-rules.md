...
本ドキュメントは、単体テスト仕様（UTS）の対象別 `uts-<term>` を統一形式で記述するためのルールです。
`uts-index` とテスト観点・条件（TPC）を前提に、対象ごとの **単体テストで保証する範囲（責務・境界・観点・合格基準）** を明文化します。

## 1. 全体方針

- 本ドキュメント（`uts-<term>`）は UTS の対象別仕様であり、独立した方針文書ではありません（`uts-index` を前提）。
- 対象は **1ドキュメント = 1論理対象**（例: 在庫、商品、注文、価格計算、割引判定）。
- **責務・境界・観点・合格基準**を明文化し、TPC 由来で追跡可能にします（曖昧表現は禁止）。
- テストケースの具体値・テストコード構造はテストコードおよび CI アーティファクトに委ねます。
...
flowchart BT
  ...
  direction RL
    ...
    UTSDetail["**uts-&lt;term&gt;<br>単体テスト仕様-対象別**"]
...
本対象が 単体テストで担保する TPC の観点を列挙します。観点IDは TPC（TP-xx）をそのまま参照します。

| 参照TPC | 観点ID | 観点分類 | 観点名   | 保証内容                         | 追加根拠（任意） |
| ------- | ------ | -------- | -------- | -------------------------------- | ---------------- |
| tpc-xxx | TP-01  | 正常     | 引当可能 | 在庫が十分な場合、引当が成功する |                  |
| tpc-xxx | TP-02  | 境界     | 在庫0    | 在庫0では引当不可と判定される    | br-xxx-xxx       |
| tpc-xxx | TP-03  | 例外     | 不正数量 | 負数指定はエラーとなる           | bac-xxx-xxx      |
| tpc-xxx | TP-04  | 状態     | 状態遷移 | 状態に応じて分岐が正しい         | cstd-xxx-xxx     |
...
代表的な 状態レベルの条件のみを示します。各観点あたり最大 3 件程度を目安とし、網羅展開・具体値はテストコードに委ねます。

| 参照TPC | 観点ID | 条件ID | 条件（状態レベル） | 期待される振る舞い |
| ------- | ------ | ------ | ------------------ | ------------------ |
| tpc-xxx | TP-01  | TC-01  | 在庫数 > 注文数    | 引当成功           |
| tpc-xxx | TP-02  | TC-02  | 在庫数 = 0         | 引当不可           |
| tpc-xxx | TP-03  | TC-03  | 注文数が負数       | エラー             |

> 数値・組合せ表・境界値展開はテストコードで表現します。
...
| 項目           | 基準（例）                          | エビデンス（例）                 |
| -------------- | ----------------------------------- | -------------------------------- |
| 重要観点       | 本対象の全観点（TP-xx）が検証済み   | テスト結果（観点IDの対応が追跡） |
...
| 参照TPC       | 観点ID | 観点分類 | 観点名   | 保証内容                                                                 | 根拠                    |
| ------------- | ------ | -------- | -------- | ------------------------------------------------------------------------ | ----------------------- |
| tpc-inventory | TP-01  | 正常     | 引当可能 | 在庫が十分な場合、対象数量が引当され、減算結果が業務ルールに一致する     | br-inventory-allocation |
| tpc-inventory | TP-02  | 異常     | 在庫不足 | 在庫不足を検知し、確定不可（またはエラー）として扱われ、更新が行われない | bac-sale-checkout       |
| tpc-inventory | TP-03  | 例外     | 入力不正 | 不正な数量/存在しない商品などは拒否され、更新が行われない                | br-inventory-allocation |

### 8.5. テスト条件（代表条件）

| 参照TPC       | 観点ID | 条件ID | 条件（状態レベル）                   | 期待される振る舞い               |
| ------------- | ------ | ------ | ------------------------------------ | -------------------------------- |
| tpc-inventory | TP-01  | TC-01  | 在庫数 > 販売数量                    | 引当成功、減算後在庫が整合する   |
| tpc-inventory | TP-02  | TC-02  | 在庫数 < 販売数量                    | 引当不可、更新が行われない       |
| tpc-inventory | TP-03  | TC-03  | 販売数量が 0 または負数 / 商品が不正 | 拒否（エラー）、更新が行われない |
...
| 項目     | 基準（例）                                  | エビデンス（例）                    |
| -------- | ------------------------------------------- | ----------------------------------- |
| 重要観点 | 参照TPC:tpc-inventoryのTP-01〜03 が検証済み | テスト結果（観点IDで追跡できる形）  |
