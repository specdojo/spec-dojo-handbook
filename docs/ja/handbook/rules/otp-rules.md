...
`otp-index` / `otp-<term>` と他ドキュメントの関係を示します。必要に応じて Mermaid を使用してよい。

- `mip-index`：移行計画のSSOT（ハイパーケア方針の骨格を含む）
- `cop-index` / `cop-*`：カットオーバー計画（切替当日の実行手順）
- `mtp-index` / `mtp-*`：移行リハーサル計画と結果（事前の実行・評価）
- `dmd-*`：データ移行設計（抽出/変換/投入/検証）
- `otp-index`：運用切替計画のSSOT（監視/窓口/当番/クローズの入口）
- `otp-*`：運用切替計画の分冊（term固有）
