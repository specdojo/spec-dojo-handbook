...
| 観点ID | 観点分類 | 観点名     | 確認したいこと                       | トレース | 適用レベル | 備考 |
| ------ | -------- | ---------- | ------------------------------------ | -------- | ---------- | ---- |
| TP-01  | 機能     | 正常処理   | 正しい入力で正しい結果が得られること | bac-xxx  | UT, IT     |      |
| TP-02  | 業務     | 業務ルール | ビジネスルールが守られていること     | br-xxx   | ST, AT     |      |
| TP-03  | 例外     | 入力不正   | 不正入力時に適切なエラーとなること   | br-xxx   | UT, IT     |      |
| TP-04  | 非機能   | 性能       | 規定時間内に処理が完了すること       | nfr-xxx  | ET, ST     |      |

### 6.3. テスト条件（必須）

- テスト条件は、観点ごとに **どのような状態・条件で確認するか**を定義します。
- 関連仕様ID（任意） は原則空でよい（TP のトレースを継承）。ただし以下の場合は 関連仕様IDは必須（MUST）とする。
  - 同一 TP 内で根拠が混在する
  - 例外条項・状態管理・非機能（タイムアウト/再送/冪等/監査）など、条件単位で根拠が重要
  - 条件ごとに適用レベルや期待結果の根拠が変わる

| 観点ID | 条件ID | 条件内容       | 前提状態 / 入力状態  | 期待される振る舞い | 適用レベル | 関連仕様ID（任意） |
| ------ | ------ | -------------- | -------------------- | ------------------ | ---------- | ------------------ |
...
| 観点ID | 観点分類 | 観点名 | 確認したいこと | トレース | 適用レベル | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| TP-01 | 機能 | 会計計算の正確性 | 商品/数量/税/割引の組み合わせでも合計が正しく算出されること | bac-sale-checkout | UT, IT |  |
| TP-02 | 業務 | 会計確定と在庫減算の整合 | 会計確定と在庫更新が矛盾しないこと（片方だけ成功しない） | bac-sale-checkout, bac-inventory-replenishment | IT, ST |  |
| TP-03 | 例外 | 在庫不足時の扱い | 在庫不足を検知し、業務として取り得る選択肢で処理できること | bac-sale-checkout | UT, IT |  |
| TP-04 | 例外 | 取消/返品時の整合性 | 取消/返品が、売上・在庫・帳票/ログの整合を崩さないこと | bac-sale-checkout | IT, ST |  |
| TP-05 | 外部I/F | 外部決済（正常/エラー）の扱い | 決済の成功/失敗/タイムアウトで、二重請求や取りこぼしが起きないこと | bac-sale-checkout, nfr-security | ET, ST |  |
| TP-06 | 非機能 | 冪等性/重複送信耐性 | 同一操作の再送があっても結果が一貫し、重複処理が発生しないこと | nfr-security | UT, IT, ET |  |
| TP-07 | セキュリティ | 権限・操作制御 | 権限に応じて会計/取消/返品が制御され、監査可能な記録が残ること | nfr-security | ST, AT | NFR |
| TP-08 | 非機能 | 性能（ピーク時の会計処理） | NFR で定義した閾値内で会計処理が完了すること | nfr-performance | ET, ST | NFR |
| TP-09 | 非機能 | 可用性（障害時の復旧/再実行） | 障害復旧後に整合性を回復でき、未確定取引の扱いが定義されていること | nfr-availability | ST, AT | SAC/NFR |
| TP-10 | データ | 帳票/出力内容の整合 | 帳票（CSV/PDF 等）に出力される内容が取引データと一致すること | bac-sale-checkout | ST, AT |  |
...
| TP-05 | TC-10 | 外部決済タイムアウト | 外部決済がタイムアウト/不達になる | 二重請求防止の方針に従って状態が管理され、再試行可否が判定できる | ET, ST | nfr-integrity |
...
| TP-08 | TC-13 | ピーク時会計性能 | ピーク想定の負荷条件（同時実行/データ量）は NFR 定義に従う | 会計処理が NFR の閾値内で完了する | ST | nfr-performance |
