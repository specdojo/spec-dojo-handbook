...
2. 対象 sample に `specdojo.grade` と `specdojo:finding` がある場合は、severity にかかわらず、各 finding の message と同じ viewpoint ID の判定根拠を修正要件として読み、指摘された問題を解消するよう該当箇所を修正する。finding コメントの削除だけを修正として扱わない。
