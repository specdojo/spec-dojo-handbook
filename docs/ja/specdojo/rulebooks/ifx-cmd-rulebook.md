...
specdojo:
  ...
  grade:
    ...
    verdict: needs-work
    score: 70
    graded_at: "2026-09-02T18:15:58.959Z"
    graded_by: codex-expert-executor
    content_hash: 2141c0c73fa125b9ed9b4728589626f06e05789c4448c8dc5b32b0308c8eab12
    categories:
    ...
    findings: { blocker: 0, major: 6, minor: 1, note: 0 }
---
...
<!-- specdojo:finding id=F001 severity=major rule=vp-arc-cross-document-consistency `version` に SemVer、`supersedes` に ID 配列を許可していますが、対応 schema は `version` を正整数に限定し、`supersedes` を許可していないため, メタ項目の契約を schema と一致させてください。 -->
...
<!-- specdojo:finding id=F004 severity=major rule=vp-qe-verifiability ルートキーだけでなく、`command`、`arguments`、`streams`、`authentication`、`execution`、`error_handling` の各入れ子キーについて必須・任意と条件付き必須を明記し、仕様だけで pass / fail を判定できるようにしてください。 -->
<!-- specdojo:finding id=F006 severity=major rule=vp-qe-omissions-consistency 対応 schema が必須とする `command.purpose`、`command.subcommands`、`command.shell`、各引数属性、各認証属性、実行属性、エラー処理属性の必須性を本文要件へ追加してください。 -->
...
<!-- specdojo:finding id=F002 severity=major rule=vp-arc-cross-document-consistency `arguments[].kind` の `positional`、`position` キー、`value_source` の `config` は対応 schema で許可されず、rulebook に従った引数定義が検証エラーになるため、許容キーと enum を統一してください。 -->
<!-- specdojo:finding id=F007 severity=major rule=vp-qe-kata-conformance rulebook と recipe が許容する `positional` および rulebook が許容する `config` を対応 schema が拒否するため、rulebook・recipe・template・schemaを通じた引数契約を統一してください。 -->
...
<!-- specdojo:finding id=F003 severity=major rule=vp-arc-cross-document-consistency 終了カテゴリとして定義した `rate_limit` は対応 schema の `exit_codes[].category` で許可されていないため、rulebook と schema の enum を一致させてください。 -->
...
<!-- specdojo:finding id=F005 severity=minor rule=vp-qe-verifiability error_handling 内で記載すべき具体的なキー名（timeout_action, malformed_output_action, authentication_failure_action 等）を明記してください。 -->
