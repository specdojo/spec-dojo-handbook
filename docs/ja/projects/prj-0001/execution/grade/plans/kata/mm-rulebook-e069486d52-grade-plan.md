---
specdojo:
  id: prj-0001:grade-kata-e069486d52c1-plan
  type: exec-plan
  rulebook: none
  task_id: GRADE-KATA-E069486D52C1
  name: "grade: docs/ja/specdojo/rulebooks/mm-rulebook.md"
  mode: review
  status: ready
  project_id: prj-0001
  targets:
    - specdojo:mm-rulebook
---

# Review Plan: GRADE-KATA-E069486D52C1 grade: docs/ja/specdojo/rulebooks/mm-rulebook.md
...
評価対象 `docs/ja/specdojo/rulebooks/mm-rulebook.md` を共通 viewpoint と category rubric で 0-4 判定する。deterministic viewpoint は CLI が判定するため、agent の出力には含めない。

## 2. 対象項目

- `target`: kata
- `rubric`: grade-rubric-v1
- `document_id`: specdojo:mm-rulebook
- `評価対象`: `docs/ja/specdojo/rulebooks/mm-rulebook.md`

### 参考資料

参考資料は評価対象ではなく、成果物間整合を判定するための材料である。参考資料自体を評価しない。

- `docs/ja/specdojo/samples/mm-sample.md`

### 良い実例（比較リファレンス）

同じ種別の `status: ready` 文書から、この plan の生成時に無作為に選んだ比較材料である。記載水準を具体化するために使い、評価対象へ含めず、内容を正解として機械的に模倣しない。`ready` は品質保証ではないため、実例自体の問題を評価対象へ転用しない。
