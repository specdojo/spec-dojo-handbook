  ...
  register_events:
    - v: 1
      ...
      changes:
        ...
        - field: title
          from: ""
          to: grade plan へ前回の finding を引き継ぎ解消を確認させる
        - field: description
          from: ""
          to: 再評価時に前回の判定結果を完全に上書きしているため、前回どの問題を指摘したかが agent に伝わらない。6 agent の比較では finding 件数が 1 件から 19 件まで分かれており、単独実行では agent ごとに網羅性が大きく異なる。前回の finding を plan へ含め、解消されているかを確認させたうえで上書きすれば、異なる agent を順に回すことで指摘を積み上げられる。maintenance が修正した後に解消を検証する経路にもなる。前回の level と score は渡さない。判定への引きずりを招き、前回が誤判定だった場合に固定化されるためである。指摘内容は事実として引き継ぎ、評価は独立して行わせる。plan には前回の指摘にない問題も検出するよう明示する。
        - field: type
          from: ""
          to: todo
        - field: priority
        ...
        - field: due
          ...
          to: "2026-09-30"
    - v: 1
      id: reg_a1b9ee36ed794bfdababe31fc2ff42cf
      ts: "2026-08-30T02:45:18Z"
      action: start
...
| No  | 作業                         | 担当   | 状態 | メモ                               |
| --- | ---------------------------- | ------ | ---- | ---------------------------------- |
| 1   | 引き継ぐ情報の範囲の決定     | ARC    | open | finding のどの項目を渡すか         |
| 2   | 観点割り当ての独立性への対処 | ARC    | open | 実験で観測した副作用への指示       |
| 3   | plan 生成の実装              | _TODO_ | open | 既存 grade からの finding 読み取り |
| 4   | 効果の確認                   | _TODO_ | open | 単独実行との比較                   |
...
- 前回の finding をどこから読むか。Frontmatter の `specdojo.grade` は件数のみを持ち、本文の `specdojo:finding` コメントが内容を持つ。コメントを解析して plan へ渡すことになる。
- 解消済みと判断された指摘を記録するか。解消の履歴は残さない方針だが、誤って解消と判断された場合に検出できなくなる。
- 引き継ぎを何世代まで続けるか。前回の finding は前々回からの引き継ぎを含みうるため、解消されない指摘が蓄積し続ける可能性がある。
