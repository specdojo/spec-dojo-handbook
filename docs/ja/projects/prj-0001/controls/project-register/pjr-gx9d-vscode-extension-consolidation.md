...
`fmt-md-table.ts` は Node 標準ライブラリだけに依存するため、拡張への取り込みに障害はない。
ただし CLI からの実行経路（`npx tsx tools/docs/src/fmt-md-table.ts`）は残す。エディタを使わない
利用者と CI が使うためである。実装を共有し、拡張はその薄いラッパーとする。
...
| 順  | 項目     | 内容                            | 依存の理由                           |
| --- | -------- | ------------------------------- | ------------------------------------ |
| 1   | PJR-0144 | `fmt-md-table` を拡張へ統合する | 拡張の機能が確定しないと公開できない |
| 2   | PJR-0143 | Marketplace へ公開する          | 公開後でないと導入手順を書けない     |
| 3   | PJR-0142 | 編集ガイドへ導入手順を追記する  | 公開先が定まってから書く             |
...
- [[prj-0001:pjr-0144-fmt-md-table-vs-code]]: 実施順序の1番目。
- [[prj-0001:pjr-0143-vs-code-marketplace]]: 実施順序の2番目。
- [[prj-0001:pjr-0142-vs-code-specdojo]]: 実施順序の3番目。
