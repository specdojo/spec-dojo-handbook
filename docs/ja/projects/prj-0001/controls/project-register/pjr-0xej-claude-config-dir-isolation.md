...
specdojo:
  ...
  conclusion: 並行実行時の ~/.claude.json 競合は Claude Code 2.1.259 で上流修正されており、2.1.268 へ更新して TUI 稼働中に claude -p を 14 回並行起動しても JSON 破損・workspace trust のリセット・MCP/project 設定の消失は発生しなかった。CLAUDE_CONFIG_DIR による設定ディレクトリの分離は不要と判断し実装しない。claude 系 executor/reporter との並行作業の実務確認は別途行い、再発時は再起票する。
---
...
| 4   | claude 系 agent と並行作業して競合が起きないことを確認する                       | ARC  | deferred  | 実務形式の確認は別途実施。再発時は再起票する |
...
- 実務形式（claude 系 executor / reporter で `exec run` を走らせながらオーケストレーターが作業する形）の確認は別途行う。競合が再発した場合は、その事象を根拠に再起票する。
