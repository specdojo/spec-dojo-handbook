...
**beads が優る点**: 依存関係のグラフを持ち、`bd ready` が未完了の依存を持たない課題だけを返す。
register に依存関係の項目はなく、`register add` にも依存を指定する選択肢がない。本セッションで
利用者から「依存関係を考慮して順番に実行ください」と指示を受けた事実が、この欠落を示している。
順序判断が人へ残っている。

schedule 側には CPM による ready 判定があるが、これは成果物カタログのタスクを対象とし、登録簿の
項目には及ばない。登録簿を backlog として使う導線を主張するなら、この差は埋める必要がある。

採用規模も大きく異なる。beads は公開から約11か月で 26,957 star を集めた。register は実質未公開
である。beads が「git ベースの agent 向け課題追跡」という枠を先に取っている。

**判定**: 登録簿の機能単体では beads が先行している。register が持つ意味は「Markdown が正本で
あること」と「kata と実行に接続していること」の2点に絞られる。前者は思想の一貫性であり、後者は
SpecDojo 全体の設計に由来する。beads は課題追跡として完成度が高いが、仕様体系は持たない。
...
| 劣位         | 学習コスト、体系の維持負担、対応 agent 数、機能量に対する成熟度、**開始の導線**、登録簿の依存関係と ready 判定 |
