...
| 区分         | 内容                                                              |
| ------------ | ----------------------------------------------------------------- |
| 独自性が高い | 実行の記帳、品質の継続評価（grade）、local LLM の段構成、保護機構 |
| 類似         | 仕様を正本とする思想、型による文脈供給、状態遷移による追跡        |
| 劣位         | 学習コスト、体系の維持負担、対応 agent 数、機能量に対する成熟度   |

SDD の入口機能では既存ツールに対抗しにくい。差別化の軸は「実行の記帳」と「品質の継続評価」に
あると考えられる。監査可能性を要する組織、長期運用する文書体系、local LLM でコストを抑えたい
場面に向く。

一方、小規模で素早く始めたい用途では OpenSpec や Spec Kit の方が適する。この位置づけが妥当かは
実際の利用者の反応を見ないと判断できない。
