...
- CLI からの実行経路（`npx tsx tools/docs/src/fmt-md-table.ts`）が引き続き動作する。エディタを
  使わない利用者と CI のために残す。
- 整形処理の実装が拡張と CLI で共有されている。両方へ複製しない。
...
- 実装の共有方法を決める。拡張が `tools/docs/src/fmt-md-table.ts` を直接 import するか、整形の
  中核を切り出して両者が参照するかを判断する。拡張は vsix へバンドルするため、リポジトリ外の
  パスへ依存できない。
