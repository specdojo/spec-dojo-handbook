...
| 070-システム受入条件 | | システム全体としての合格基準を定義 | 機能・非機能・障害・移行などの受け入れ条件 |
...
| 070-システム受入条件 | 「性能試験で全てのNFRを満たし、重大障害0件であること。」 |
...
| ドキュメント | 目的 | 主な内容 |
| --- | --- | --- |
| 010-テスト戦略・方針 | 全体テストの考え方を示す（要件） | テストレベルと目的、スコープ、環境、フェーズ間の入口／出口条件 |
| 020-テスト観点・テスト条件 | 何を確認すべきかを整理（仕様） | 業務観点、例外観点、非機能観点、要件・機能IDとのトレース |
| 030-単体テスト | 単体テスト設計・結果を記録（設計・実装・結果） | クラス／メソッド単位のテストケース・期待値・結果 |
| 040-結合テスト | コンポーネント間結合の検証（設計・実装・結果） | 結合パターン、ケース、期待値、結果 |
| 050-総合テスト | システム全体の動作確認（設計・実装・結果） | 業務シナリオテスト、非機能テスト結果 |
| 060-受入テスト | ユーザー受け入れ結果を記録（設計・実装・結果） | 業務受け入れ条件との対応、判定、指摘・要望 |
